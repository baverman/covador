...
test -t 1 && DTTY='-t'
mkdir -p ~/.cache/covador
docker run -i $DTTY --rm -w /build -u $user -v $PWD:/build -v ~/.cache/covador:/tmp covador ./ci/test.sh
docker run -i $DTTY --rm -w /build \
           -e PIP_OPTS="--cache-dir=/tmp/pip" \
           -u $user -v $PWD:/build -v ~/.cache/covador:/tmp \
